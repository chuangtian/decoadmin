...
export default function extension() {
  render(<OrderStatusRecommendations />, document.body);
}

function OrderStatusRecommendations() {
  const api = useApi();
  ...
  useEffect(() => {
    ...
    setCandidates([]);
    fetchOrderStatusConfiguration(api, configurationMetafields)
  ...
  useEffect(() => {
    ...
    fetchRecommendations(api, configuration, lines, {
      market: market?.handle ?? '',
      currency: currency?.isoCode ?? '',
      cartSubtotal: totalAmount?.amount ?? null,
  ...
  }, [configuration, lineSignature, market?.handle, currency?.isoCode, totalAmount?.amount, language?.isoCode]);
            ...
            {current.discount?.percentage
              ...
              ? <s-stack gap="small-100">
                  ...
                  <s-stack direction="inline" gap="small-200" alignItems="center">
                    <s-text type="strong">{formatMoney(current.discounted_amount, current.currency, language?.isoCode ?? '')}</s-text>
                    <s-text type="redundant">{formatMoney(current.amount, current.currency, language?.isoCode ?? '')}</s-text>
                  </s-stack>
                </s-stack>
              : <s-text>{formatMoney(current.amount, current.currency, language?.isoCode ?? '')}</s-text>}
          </s-stack>
          <s-button
            variant="primary"
