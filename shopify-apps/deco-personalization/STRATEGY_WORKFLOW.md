...
Smart Cart follows the same separation: merchants create and maintain the reusable recommendation template under **策略**, then select exactly one strategy and a heading under **概览 → Smart Cart**. The strategy list derives `Smart Cart` under “用于” from that real Store-scoped binding. Saving a different strategy resets the compatibility/preview gate and keeps Smart Cart off until the replacement is retested. Deleting a bound strategy transactionally clears and disables the Smart Cart binding before deleting the strategy, so the storefront falls back to the Shopify cart without a broken recommendation frame.

The Smart Cart surface displays only the first eligible strategy result. Its card includes the selected variant image, wrapping product/variant text, configured minimum quantity, and either one normal price or a struck-through original price plus the validated discounted price. A successful add refreshes the cart and strategy result before displaying the next product; items already in the cart are filtered again in the storefront runtime, and the complete recommendation region disappears when no eligible candidate remains. Add failure keeps the drawer usable and retryable instead of redirecting or blocking shopping.
...
Listing requires `read_discounts`; creation and editing require `write_discounts`. Both scopes belong only to the independent Personalization App and must be granted through the Test installation before discount management is available. For an active percentage discount, the recommendation response includes the original amount, discount percentage, discounted amount, currency, and code. Checkout applies it through the Checkout UI API. Smart Cart uses Shopify's locale-aware Ajax Cart API and `/cart/update.js` `discount` parameter, retaining any detectable existing code before adding the strategy code. A missing, inactive, or rejected discount never breaks the underlying recommendation, cart, or checkout.
