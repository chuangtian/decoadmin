...
class InvitationEmail
{
    public function content(Store $store, ?Invitation $invite = null, string $kind = 'initial'): array
    {
        ...
        abort_unless(in_array($kind, ['initial', 'reminder'], true), 422);
        if ($invite) {
        ...
        return ['storeName' => $store->name, 'productName' => $variables['{product}'], 'preview' => $invite === null,
            'subject' => str_replace(["\r", "\n"], ' ', strtr($settings[$kind === 'reminder' ? 'reminder_subject' : 'subject'], $variables)),
            'body' => strtr($settings[$kind === 'reminder' ? 'reminder_body' : 'email_body'], $variables),
