...
class DispatchInvitations extends Command
{
    ...
    public function handle(): int
    {
        ...
        foreach ($stores as $store) {
            ...
            if (! $settings['invites_enabled'] || ! $settings['reminders_enabled']) {
                continue;
            }
            Invitation::where('organization_id', $store->organization_id)->where('store_id', $store->id)
                ->where('status', 'sent')->whereNull('reminder_sent_at')->where('expires_at', '>', now())
                ->where('sent_at', '<=', now()->subDays($settings['reminder_days']))->where('updated_at', '<', now()->subMinutes(5))
