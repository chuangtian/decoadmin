...
The current test build includes a default-off Happy Customers App Proxy page that aggregates only published product and store reviews. It uses the same public-field allow-list and pagination as the storefront feed; disabling either public review display or the dedicated page switch makes the route unavailable.

Product review groups provide tenant-scoped shared display for related products without changing Shopify products or the original ownership of a review. Each product belongs to at most one retained group; disabling the group restores individual product feeds, while invitations, rewards, moderation and form targeting stay product-specific.

The theme block now exposes isolated storefront branding controls for grid/list/mosaic layout, three corner styles, stars, buttons, card surfaces and text. Runtime and theme-editor labels are complete for English, Simplified Chinese, German, French and Spanish; these built-in translations do not claim merchant-managed per-locale custom copy. An optional after-submit page accepts only a Shopify theme URL and the storefront script enforces the current origin before navigating.

Implemented: scoped DecoAdmin review management; product/store reviews; rating-neutral moderation and scheduled publishing; merchant replies and featured reviews; distinct order verification, reversible merchant verification, import origin and unverified storefront disclosures; encrypted private email; image re-encoding and manual video moderation; signed order-bound buyer forms; independently enabled signed app-proxy product and store review collection with mandatory moderation; exact product/fulfillment invitation verification; separate request and photo/video reminders; product/store thank-you and public-reply lifecycle messages with encrypted recipients, idempotency, audit and SMTP uncertainty hold; explicit per-store automation gates; provider-aware CSV import/deduplication/seven-day withdrawal; formula-safe export; nine storefront display modes and media viewer. Carousel controls and opt-in accessible autoplay, gallery main-media/thumbnail presentation and keyboard navigation, a bounded video-review slider, sidebar/floating disclosure, per-block lifecycle state, keyboard focus restoration, reduced-motion behavior, error cleanup and theme-section teardown are released to the test app; this is not a claim of 17-widget parity.
...
Reward increment (released to test; not production): an eligible published photo/video review from an order-verified email invitation can create one non-combinable, one-use Shopify percentage, fixed-amount, or free-shipping code. Records are store-scoped and idempotent, codes and delivery recipients are encrypted and excluded from management history, Shopify writes require a separate fail-closed environment gate, and uncertain results are held without retry. A confirmed issue schedules one reward email; an optional reminder is scheduled once only after the first email is confirmed sent and before expiry. Administrators can explicitly record held Shopify or email results without triggering another external call. HMAC-verified order webhooks track redemption, later refunds and order cancellation using only scoped hashes and identifiers; raw webhook payloads are not retained.

Not yet complete: all 17 Loox widgets and editors, collection-targeted review links, product groups/bundles, reward revocation/reissue policy, referrals, AI/Studio, review syndication and Merchant API/webhooks. No placeholder is presented as a working external integration.
...
- No real customer mail, production discount changes, or real-store order automation. Test environment defaults to sending disabled, empty store allowlist and empty recipient allowlist. A one-shot synthetic reward acceptance is permitted only for the explicitly authorized test store and internal recipient, after which settings and gates must be restored. The draft theme block cannot enable any automation.
...
- CSV import supports the Deco custom schema plus core text fields from Loox, Judge.me, Yotpo, Okendo and Shopify Product Reviews. The provider must be selected explicitly unless the header matches exactly one known signature. Maximum 15 MB and 1,000 rows per batch. Imports never fetch remote media, never infer verified purchase from CSV assertions, and never resolve a handle as a numeric Shopify Product ID. A scoped failure report exports only line numbers and stable error codes; undo withdraws records rather than deleting audit history. The authorized management export includes bounded immutable custom-answer snapshots in one structured JSON column, including explicit public/private visibility, without changing historical question columns.
