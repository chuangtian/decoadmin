...
## Provider import and widget lifecycle test release

- Backend feature commit `cc520910c45516af70f6d8caa0d815d7fb5abdf2` was deployed from a clean `origin/test` archive. Backup: `/opt/decoadmin/backups/deco-reviews-form-cc520910c45516af70f6d8caa0d815d7fb5abdf2`. Migration `2026_09_13_080000_add_provider_to_deco_review_imports` completed. Application, database, Redis and Horizon passed; the release script verified production containers were unchanged.
- Shopify test version `deco-reviews-test-10`, ID `1127020527617`, was released only for **Deco Reviews (Test)**. No Shopify theme was published.
- Provider selection now supports Deco custom CSV, Loox, Judge.me, Yotpo, Okendo and Shopify Product Reviews core text fields. Automatic detection fails closed on unknown or ambiguous headers; remote media is not downloaded; provider and batch attribution are retained.
- Reward-email results held after an uncertain SMTP outcome can now be explicitly reconciled as sent or not sent under scoped management permission. This records an audit conclusion and never resends the message.
- Carousel autoplay is opt-in and pauses or stops for reduced motion, hover, focus, page visibility and manual interaction. Gallery keyboard navigation, current-item semantics, multi-block isolation, duplicate-script guard and section teardown/reload behavior are covered.
- Final automated evidence: 80 Deco Reviews backend tests / 544 assertions and 18 storefront JavaScript tests passed. Type-check, interface and responsive typography checks, app assets, Shopify config validation, Shopify App build and DecoAdmin production frontend build passed. Every generated extension JavaScript asset remains below 10 KB.
- Chrome visual acceptance could not be completed after release because the browser-control connection repeatedly failed to load its request-header policy. This release is server- and automation-verified, not yet visually accepted in the authorized Horizon draft theme.
