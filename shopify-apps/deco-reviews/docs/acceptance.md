...
- Shopify test versions `deco-reviews-test-9` (reward closure), `deco-reviews-test-10` (provider imports and widget lifecycle), and `deco-reviews-test-11` (reward redemption/refund/cancellation webhooks, version ID `1127108214785`) are released. The current macfox-test-app token was verified to include `read_discounts` and `write_discounts`.
