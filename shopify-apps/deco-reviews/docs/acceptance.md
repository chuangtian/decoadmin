...
| Email and scheduling | Seven independently configured message stages now exist, including reward delivery and one optional reward reminder. Recipients are encrypted, delivery is idempotent, and uncertain SMTP results are held without blind retry. | Invitation, lifecycle, reward delivery/reminder, encryption, allow-list, idempotency and held-result tests are present. | Implemented subset; pending live reward-email verification | Complete global style/per-message overrides, localization, order-tag exclusions, bounce/complaint handling, delivery analytics, and held-email reconciliation. Real-customer sending remains forbidden. |
...
| Storefront base widget | Native JavaScript renders aggregate stars, histogram, review cards, sorting/rating filtering, pagination, replies, media, disclosure badges, and accessible lightbox navigation. Feed settings drive star color, layout, heading, and corner style. | Thirteen storefront tests cover form behavior, disclosure, carousel controls, keyboard/focus behavior, reduced motion, sidebar disclosure, gallery empty state, failure cleanup and section teardown. | Implemented; pending draft-theme browser verification | Verify responsive visual layout, multiple blocks per page, theme-editor reload, native media failures and real focus behavior in the authorized draft theme. |
| Widget mode prototypes | Theme block exposes `reviews`, `stars`, `carousel`, `trust`, `snippets`, `gallery`, `video`, `sidebar`, and `floating` modes. Carousel has manual controls; sidebar/floating have accessible launcher/close behavior; gallery suppresses text-only empty shells. | Storefront interaction tests cover the newly interactive modes. | Improved prototype subset | These modes do not equal the audited 17 Loox widgets. Autoplay policy, gallery thumbnail navigation, launcher persistence/frequency, separate editors and independent theme acceptance remain pending. |
...
- Deco Reviews automated suite: 73 tests, 496 assertions passed.
- Storefront JavaScript suite: 13 tests passed; every generated JavaScript asset remains below the 10 KB extension limit.
...
- Shopify test version `deco-reviews-test-8` is released, and the current macfox-test-app token was verified to include `read_discounts` and `write_discounts`.
