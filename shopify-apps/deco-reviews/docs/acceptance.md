...
| Import and export | Deco custom CSV plus Loox, Judge.me, Yotpo, Okendo and Shopify Product Reviews core text mappings are bounded to 15 MB and 1,000 rows per batch. Explicit provider selection, unambiguous-only detection, scoped product resolution, batch attribution, deduplication, seven-day reversible withdrawal, and formula-safe streamed export are implemented. | Provider mapping/detection, authorization, tenant isolation, numeric-handle disambiguation, management validation, import errors/deduplication/undo and spreadsheet formula tests are present. | Implemented core-provider subset | Remote image import, full provider fields, 100,000-review lifecycle, async chunking/progress, downloadable failure report, store reviews, replies, verification/reward flags, custom questions and historical provider mappings remain pending. |
...
- Shopify test version `deco-reviews-test-9` is released, and the current macfox-test-app token was verified to include `read_discounts` and `write_discounts`.
