...
type Review = {
    ...
    body: string; status: ReviewStatus; featured: boolean; source: 'merchant' | 'email' | 'import' | 'organic'; verified_source: 'none' | 'order' | 'import' | 'manual';
...
type RewardHistory = { uuid: string; media_kind: 'photo' | 'video'; discount_kind: 'percentage' | 'fixed' | 'free_shipping'; value: string | null; currency: string | null; status: string; review_title?: string | null; product_title?: string | null; due_at?: string | null; issued_at?: string | null; redeemed_at?: string | null; redemption_order_number?: string | null; refunded_at?: string | null; cancelled_order_at?: string | null; expires_at?: string | null; created_at: string; error_code?: string | null; email_status?: string | null; email_delivery_uuid?: string | null; email_sent_at?: string | null; email_error_code?: string | null; reminder_status?: string | null; reminder_delivery_uuid?: string | null; reminder_sent_at?: string | null; reminder_error_code?: string | null };
...
type Settings = {
    ...
    reward_issued_subject: string; reward_issued_body: string; reward_reminder_enabled: boolean; reward_reminder_days: number;
    reward_reminder_subject: string; reward_reminder_body: string;
};
type FormConfig = {
...
const editForm = useForm({ status: 'pending' as ReviewStatus, featured: false, reply: '', reason: '', manual_verification: false });
...
const statusLabel = (status: string) => ({ pending: '待审核', published: '已发布', unpublished: '未发布', verification_required: '待验证履约与发送条件', scheduled: '已排期', issued: '已在 Shopify 创建', redeemed: '已使用', issuing: '正在创建', sent: '已发送', sending: '正在发送', held: '结果待复核', sending_reminder: '正在发送提醒', reminder_held: '提醒发送结果待复核', sending_media_reminder: '正在发送媒体提醒', media_reminder_held: '媒体提醒结果待复核', unsubscribed: '已退订', cancelled: '已取消', completed: '已完成', processing: '处理中', failed: '失败', undone: '已撤销' }[status] || status);
const statusClass = (status: string) => ({ published: 'bg-emerald-50 text-emerald-700', issued: 'bg-emerald-50 text-emerald-700', redeemed: 'bg-blue-50 text-blue-700', sent: 'bg-emerald-50 text-emerald-700', pending: 'bg-amber-50 text-amber-700', held: 'bg-amber-50 text-amber-800', reminder_held: 'bg-amber-50 text-amber-800', media_reminder_held: 'bg-amber-50 text-amber-800', unpublished: 'bg-slate-100 text-slate-700', failed: 'bg-red-50 text-red-700', cancelled: 'bg-slate-100 text-slate-600' }[status] || 'bg-blue-50 text-blue-700');
...
const verificationLabel = (source: Review['verified_source']) => ({ order: '订单验证购买', manual: '商家人工核验', import: '导入来源', none: '未验证' }[source]);
const reviewSourceLabel = (source: string) => ({ email: '邀评邮件', organic: '公开表单', merchant: '手动创建', import: '数据导入' }[source] || source);
...
const openEditor = (review: Review) => {
    ...
    editForm.manual_verification = review.verified_source === 'manual';
...
const reconcileReward = (reward: RewardHistory, conclusion: 'created' | 'not_created') => {
    const message = conclusion === 'created' ? '确认你已在 Shopify 核对并看到该优惠码已创建？系统只记录结论，不会再次调用 Shopify。' : '确认你已在 Shopify 核对并确认该优惠码未创建？该奖励会标记失败且不会自动重试。';
    if (!window.confirm(message)) return;
    router.post(`${props.baseUrl}/rewards/${reward.uuid}/reconcile`, { conclusion }, { preserveScroll: true, onSuccess: () => { notice.value = '人工核对结论已记录，没有重试 Shopify。'; }, onError: () => { notice.value = ''; } });
};
const reconcileRewardDelivery = (deliveryUuid: string | null | undefined, conclusion: 'sent' | 'not_sent') => {
    if (!deliveryUuid) return;
    const message = conclusion === 'sent' ? '确认你已在邮件服务商中核对并确认邮件已发送？系统只记录结论，不会再次发送。' : '确认你已核对并确认邮件未发送？该邮件会标记失败且不会自动重试。';
    if (!window.confirm(message)) return;
    router.post(`${props.baseUrl}/reward-deliveries/${deliveryUuid}/reconcile`, { conclusion }, { preserveScroll: true, onSuccess: () => { notice.value = '邮件人工核对结论已记录，没有重新发送。'; }, onError: () => { notice.value = ''; } });
};
...
<template>
    ...
    <AppLayout :breadcrumbs="[{ label: '工作台', href: '/dashboard' }, { label: '应用中心' }, { label: 'Deco Reviews' }]">
        <div class="mx-auto max-w-7xl space-y-6 text-[14px] text-slate-800">
                    ...
                    <form class="mt-4 grid gap-3 md:grid-cols-5" @submit.prevent="applyFilters">
                        ...
                        <details class="rounded-xl border border-slate-200 bg-slate-50 p-4 md:col-span-5">
                            ...
                            <div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                                ...
                                <label><span class="text-[12px] font-semibold text-slate-600">来源</span><select v-model="query.source" class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2"><option value="">全部</option><option value="email">邀评邮件</option><option value="organic">店铺公开表单</option><option value="merchant">手动创建</option><option value="import">导入</option></select></label>
                                <label><span class="text-[12px] font-semibold text-slate-600">真实性状态</span><select v-model="query.verified" class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2"><option value="">全部</option><option value="order">订单验证购买</option><option value="manual">商家人工核验</option><option value="none">未验证</option></select></label>
                    ...
                    <article v-for="review in reviews.data" :key="review.uuid" class="border-b border-slate-100 p-5 last:border-0">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-2"><span class="tracking-wider text-amber-500" :aria-label="`${review.rating} 星`">{{ stars(review.rating) }}</span><span class="rounded-full px-2.5 py-1 text-[12px] font-semibold" :class="statusClass(review.status)">{{ statusLabel(review.status) }}</span><span v-if="review.featured" class="rounded-full bg-violet-50 px-2.5 py-1 text-[12px] font-semibold text-violet-700">精选</span><span class="rounded-full px-2.5 py-1 text-[12px] font-semibold" :class="review.verified_source === 'order' ? 'bg-emerald-50 text-emerald-700' : review.verified_source === 'manual' ? 'bg-indigo-50 text-indigo-700' : 'bg-slate-100 text-slate-600'">{{ verificationLabel(review.verified_source) }}</span><span v-if="review.incentivized" class="rounded-full bg-orange-50 px-2.5 py-1 text-[12px] font-semibold text-orange-700">激励评价</span></div>
            ...
            <template v-else-if="tab === 'invitations'">
                <section class="grid gap-5 lg:grid-cols-3">
                    ...
                    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:col-span-2">
                        ...
                        <div v-for="invitation in invitations.data" :key="invitation.uuid" class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 last:border-0">
                            <div><p class="font-semibold text-slate-950">订单 {{ invitation.order_number || `#${invitation.order_id}` }}</p><p v-if="invitation.product_title" class="mt-1 text-slate-600">{{ invitation.product_title }}</p><p class="mt-1 text-slate-500">{{ invitation.sent_at ? `初次邮件发送于 ${formatDate(invitation.sent_at)}` : invitation.due_at ? `计划于 ${formatDate(invitation.due_at)}` : `创建于 ${formatDate(invitation.created_at)}` }}</p><p v-if="invitation.reminder_sent_at" class="mt-1 text-slate-500">普通提醒发送于 {{ formatDate(invitation.reminder_sent_at) }}</p><p v-if="invitation.media_reminder_sent_at" class="mt-1 text-slate-500">图片/视频提醒发送于 {{ formatDate(invitation.media_reminder_sent_at) }}</p><p v-if="invitation.completed_at" class="mt-1 text-slate-500">评价提交于 {{ formatDate(invitation.completed_at) }}</p><p v-if="invitation.error_code" class="mt-1 text-[12px] text-slate-500">条件代码：{{ invitation.error_code }}</p></div>
                ...
                <section class="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <div class="border-b border-slate-200 px-5 py-4"><h2 class="text-lg font-semibold text-slate-950">评价生命周期邮件</h2><p class="mt-1 text-slate-500">不显示收件人地址；发送结果不确定时会停在待复核状态，不会盲目重试。</p></div>
                    <div v-if="!emailDeliveries.length" class="px-6 py-12 text-center text-slate-500">还没有评价感谢或公开回复通知记录。</div>
                    <div v-for="delivery in emailDeliveries" :key="delivery.uuid" class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 last:border-0">
                        <div><p class="font-semibold text-slate-950">{{ emailTypeLabel(delivery.type) }}</p><p class="mt-1 text-slate-600">{{ delivery.review_title || delivery.product_title || '店铺评价' }}</p><p class="mt-1 text-slate-500">{{ delivery.sent_at ? `发送于 ${formatDate(delivery.sent_at)}` : delivery.due_at ? `计划于 ${formatDate(delivery.due_at)}` : `创建于 ${formatDate(delivery.created_at)}` }}</p><p v-if="delivery.error_code" class="mt-1 text-[12px] text-slate-500">结果代码：{{ delivery.error_code }}</p></div>
                ...
                <section class="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div v-for="reward in rewardHistory" :key="reward.uuid" class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 last:border-0">
                        <div><p class="font-semibold text-slate-950">{{ reward.media_kind === 'video' ? '视频评价奖励' : '图片评价奖励' }} · {{ rewardLabel(reward) }}</p><p class="mt-1 text-slate-600">{{ reward.review_title || reward.product_title || '商品评价' }}</p><p class="mt-1 text-slate-500">{{ reward.issued_at ? `创建于 Shopify：${formatDate(reward.issued_at)}` : reward.due_at ? `计划处理：${formatDate(reward.due_at)}` : `记录创建：${formatDate(reward.created_at)}` }}<span v-if="reward.expires_at"> · 到期：{{ formatDate(reward.expires_at) }}</span></p><p v-if="reward.redeemed_at" class="mt-1 text-blue-700">已使用：{{ formatDate(reward.redeemed_at) }}<span v-if="reward.redemption_order_number"> · 订单 {{ reward.redemption_order_number }}</span></p><p v-if="reward.refunded_at" class="mt-1 text-amber-700">使用该奖励的订单发生退款：{{ formatDate(reward.refunded_at) }}</p><p v-if="reward.cancelled_order_at" class="mt-1 text-amber-700">使用该奖励的订单已取消：{{ formatDate(reward.cancelled_order_at) }}</p><p v-if="reward.email_status" class="mt-1 text-slate-500">奖励邮件：{{ statusLabel(reward.email_status) }}<span v-if="reward.email_sent_at"> · {{ formatDate(reward.email_sent_at) }}</span></p><p v-if="reward.reminder_status" class="mt-1 text-slate-500">到期提醒：{{ statusLabel(reward.reminder_status) }}<span v-if="reward.reminder_sent_at"> · {{ formatDate(reward.reminder_sent_at) }}</span></p><p v-if="reward.error_code || reward.email_error_code || reward.reminder_error_code" class="mt-1 text-[12px] text-slate-500">结果代码：{{ [reward.error_code, reward.email_error_code, reward.reminder_error_code].filter(Boolean).join(' · ') }}</p></div>
                        <div class="flex flex-wrap items-center gap-3"><span class="rounded-full px-2.5 py-1 text-[12px] font-semibold" :class="statusClass(reward.status)">{{ statusLabel(reward.status) }}</span><template v-if="canManage && reward.status === 'held'"><button type="button" class="rounded-lg border border-emerald-200 px-3 py-2 font-semibold text-emerald-700" @click="reconcileReward(reward, 'created')">核对为已创建</button><button type="button" class="rounded-lg border border-red-200 px-3 py-2 font-semibold text-red-700" @click="reconcileReward(reward, 'not_created')">核对为未创建</button></template><template v-if="canManage && reward.email_status === 'held'"><button type="button" class="rounded-lg border border-emerald-200 px-3 py-2 font-semibold text-emerald-700" @click="reconcileRewardDelivery(reward.email_delivery_uuid, 'sent')">邮件已发送</button><button type="button" class="rounded-lg border border-red-200 px-3 py-2 font-semibold text-red-700" @click="reconcileRewardDelivery(reward.email_delivery_uuid, 'not_sent')">邮件未发送</button></template><template v-if="canManage && reward.reminder_status === 'held'"><button type="button" class="rounded-lg border border-emerald-200 px-3 py-2 font-semibold text-emerald-700" @click="reconcileRewardDelivery(reward.reminder_delivery_uuid, 'sent')">提醒已发送</button><button type="button" class="rounded-lg border border-red-200 px-3 py-2 font-semibold text-red-700" @click="reconcileRewardDelivery(reward.reminder_delivery_uuid, 'not_sent')">提醒未发送</button></template></div>
                    </div>
                </section>
            </template>

            <template v-else-if="tab === 'form'">
                <ReviewFormEditor :base-url="baseUrl" :can-manage="canManage" :products="products" :form-config="formConfig" @dirty="formEditorDirty = $event" @saved="notice = '评价表单已保存。'" />
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <h2 class="text-lg font-semibold text-slate-950">店铺评价公开链接</h2>
                    <p class="mt-2 text-slate-500">用于收集不绑定具体商品的店铺评价。公开提交始终进入待审核，并标记为未验证购买。</p>
                    <div v-if="storeReviewUrl" class="mt-5 flex flex-col gap-3 sm:flex-row">
                        <input :value="storeReviewUrl" readonly aria-label="店铺评价链接" class="min-h-11 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3" @focus="($event.target as HTMLInputElement).select()" />
                        <button type="button" class="rounded-xl border border-slate-300 px-5 py-2.5 font-semibold" @click="copyStoreReviewUrl">复制链接</button>
                        <a :href="storeReviewUrl" target="_blank" rel="noopener" class="rounded-xl bg-violet-700 px-5 py-2.5 text-center font-semibold text-white">打开链接</a>
                    </div>
            ...
            <form v-else-if="tab === 'settings'" class="space-y-5" @submit.prevent="saveSettings">
                ...
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex flex-wrap items-start justify-between gap-3"><div><h2 class="text-lg font-semibold text-slate-950">图片/视频提醒邮件</h2><p class="mt-2 text-slate-500">普通提醒之后仍未提交评价时，可再发送一次鼓励附加图片或视频的邮件；无媒体也允许评价。</p></div><a :href="`${baseUrl}/email-preview?kind=media_reminder`" target="_blank" rel="noopener" class="rounded-xl border border-slate-200 px-4 py-2.5 font-semibold text-violet-700">预览邮件</a></div>
                    <div class="mt-5 grid gap-5 md:grid-cols-2"><label class="font-medium">距普通提醒天数<input v-model.number="settingsForm.media_reminder_days" :disabled="!canManage" type="number" min="1" max="90" required class="mt-2 w-full rounded-xl border px-3 py-2.5" /></label><label class="font-medium">主题<input v-model="settingsForm.media_reminder_subject" :disabled="!canManage" maxlength="160" class="mt-2 w-full rounded-xl border px-3 py-2.5" /></label><label class="font-medium md:col-span-2">正文<textarea v-model="settingsForm.media_reminder_body" :disabled="!canManage" rows="5" class="mt-2 w-full rounded-xl border px-3 py-2.5"></textarea></label></div>
                </section>
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex flex-wrap items-start justify-between gap-3"><div><h2 class="text-lg font-semibold text-slate-950">商品评价感谢邮件</h2><p class="mt-2 text-slate-500">商品评价提交成功后只建立一条幂等发送记录。</p></div><a :href="`${baseUrl}/email-preview?kind=product_thank_you`" target="_blank" rel="noopener" class="rounded-xl border border-slate-200 px-4 py-2.5 font-semibold text-violet-700">预览邮件</a></div>
                    <label class="mt-5 flex items-center gap-2 font-semibold"><input v-model="settingsForm.product_thank_you_enabled" :disabled="!canManage" type="checkbox" />启用商品评价感谢邮件</label>
                    <div class="mt-5 grid gap-5 md:grid-cols-2"><label class="font-medium">主题<input v-model="settingsForm.product_thank_you_subject" :disabled="!canManage" maxlength="160" class="mt-2 w-full rounded-xl border px-3 py-2.5" /></label><label class="font-medium md:col-span-2">正文<textarea v-model="settingsForm.product_thank_you_body" :disabled="!canManage" rows="5" class="mt-2 w-full rounded-xl border px-3 py-2.5"></textarea></label></div>
                </section>
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex flex-wrap items-start justify-between gap-3"><div><h2 class="text-lg font-semibold text-slate-950">店铺评价感谢邮件</h2><p class="mt-2 text-slate-500">店铺评价与商品评价使用独立开关和文案。</p></div><a :href="`${baseUrl}/email-preview?kind=store_thank_you`" target="_blank" rel="noopener" class="rounded-xl border border-slate-200 px-4 py-2.5 font-semibold text-violet-700">预览邮件</a></div>
                    <label class="mt-5 flex items-center gap-2 font-semibold"><input v-model="settingsForm.store_thank_you_enabled" :disabled="!canManage" type="checkbox" />启用店铺评价感谢邮件</label>
                    <div class="mt-5 grid gap-5 md:grid-cols-2"><label class="font-medium">主题<input v-model="settingsForm.store_thank_you_subject" :disabled="!canManage" maxlength="160" class="mt-2 w-full rounded-xl border px-3 py-2.5" /></label><label class="font-medium md:col-span-2">正文<textarea v-model="settingsForm.store_thank_you_body" :disabled="!canManage" rows="5" class="mt-2 w-full rounded-xl border px-3 py-2.5"></textarea></label></div>
                </section>
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex flex-wrap items-start justify-between gap-3"><div><h2 class="text-lg font-semibold text-slate-950">公开回复通知</h2><p class="mt-2 text-slate-500">仅在已发布评价首次保存公开回复时建立通知；重复保存不会重复发送。</p></div><a :href="`${baseUrl}/email-preview?kind=reply_notification`" target="_blank" rel="noopener" class="rounded-xl border border-slate-200 px-4 py-2.5 font-semibold text-violet-700">预览邮件</a></div>
                ...
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <p class="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-900">每条评价最多创建一个一次性 Shopify 优惠码，且不与其他优惠叠加。Shopify 返回结果不确定时会停止并等待人工复核；必须人工确认“已创建”或“未创建”，系统不会重复调用 Shopify。</p>
                </section>
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex flex-wrap items-start justify-between gap-3"><div><h2 class="text-lg font-semibold text-slate-950">奖励发放邮件</h2><p class="mt-2 text-slate-500">优惠码确认创建后建立一条加密收件人的幂等邮件记录；发送结果不确定时停止待复核。</p></div><a :href="`${baseUrl}/email-preview?kind=reward_issued`" target="_blank" rel="noopener" class="rounded-xl border border-slate-200 px-4 py-2.5 font-semibold text-violet-700">预览邮件</a></div>
                    <div class="mt-5 grid gap-5 md:grid-cols-2"><label class="font-medium">主题<input v-model="settingsForm.reward_issued_subject" :disabled="!canManage" maxlength="160" class="mt-2 w-full rounded-xl border px-3 py-2.5" /></label><label class="font-medium md:col-span-2">正文<textarea v-model="settingsForm.reward_issued_body" :disabled="!canManage" rows="5" class="mt-2 w-full rounded-xl border px-3 py-2.5"></textarea></label></div>
                </section>
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <div class="flex flex-wrap items-start justify-between gap-3"><div><h2 class="text-lg font-semibold text-slate-950">奖励到期提醒</h2><p class="mt-2 text-slate-500">仅在奖励邮件已确认发送、优惠码仍有效且尚未建立提醒时，排期一次提醒。</p></div><a :href="`${baseUrl}/email-preview?kind=reward_reminder`" target="_blank" rel="noopener" class="rounded-xl border border-slate-200 px-4 py-2.5 font-semibold text-violet-700">预览提醒</a></div>
                    <label class="mt-5 flex items-center gap-2 font-semibold"><input v-model="settingsForm.reward_reminder_enabled" :disabled="!canManage" type="checkbox" />启用一次奖励到期提醒</label>
                    <div class="mt-5 grid gap-5 md:grid-cols-2"><label class="font-medium">发放邮件后等待天数<input v-model.number="settingsForm.reward_reminder_days" :disabled="!canManage || !settingsForm.reward_reminder_enabled" type="number" min="1" :max="Math.max(1, settingsForm.reward_expiration_days - 1)" required class="mt-2 w-full rounded-xl border px-3 py-2.5" /></label><label class="font-medium">主题<input v-model="settingsForm.reward_reminder_subject" :disabled="!canManage" maxlength="160" class="mt-2 w-full rounded-xl border px-3 py-2.5" /></label><label class="font-medium md:col-span-2">正文<textarea v-model="settingsForm.reward_reminder_body" :disabled="!canManage" rows="5" class="mt-2 w-full rounded-xl border px-3 py-2.5"></textarea></label></div>
                    <p class="mt-3 text-[12px] text-slate-500">可用变量：{store}、{product}、{author}、{value}、{expires}、{code}。优惠码始终会在邮件中单独显示。</p>
                </section>
            ...
            <template v-else-if="tab === 'imports'">
                <section class="grid gap-5 lg:grid-cols-3"><form class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm" @submit.prevent="submitImport"><h2 class="text-lg font-semibold text-slate-950">导入 CSV</h2><p class="mt-2 text-slate-500">导入时会检查重复记录并安全跳过，不会覆盖已有评价，也不会下载第三方媒体。</p><label class="mt-5 block font-medium">CSV 来源<select v-model="importForm.provider" :disabled="!canManage" required class="mt-2 block min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3"><option value="custom">自定义 Deco CSV</option><option value="auto">自动识别（仅限明确表头）</option><option value="loox">Loox</option><option value="judge_me">Judge.me</option><option value="yotpo">Yotpo</option><option value="okendo">Okendo</option><option value="shopify_product_reviews">Shopify Product Reviews</option></select></label><p v-if="importForm.errors.provider" class="mt-2 text-red-700">{{ importForm.errors.provider }}</p><label class="mt-5 block font-medium">CSV 文件<input :disabled="!canManage" required accept=".csv,text/csv" type="file" class="mt-2 block w-full rounded-xl border border-slate-200 p-3" @change="importForm.file = ($event.target as HTMLInputElement).files?.[0] || null" /></label><p v-if="importForm.errors.file" class="mt-2 text-red-700">{{ importForm.errors.file }}</p><button :disabled="!canManage || importForm.processing" class="mt-5 w-full rounded-xl bg-violet-700 px-5 py-3 font-semibold text-white disabled:opacity-50">{{ importForm.processing ? '上传中…' : '上传并导入' }}</button></form><section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:col-span-2"><div class="border-b px-5 py-4"><h2 class="text-lg font-semibold text-slate-950">导入历史</h2></div><div v-if="!imports.length" class="px-6 py-16 text-center text-slate-500">还没有导入记录。</div><div v-for="row in imports" :key="row.uuid" class="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 px-5 py-4 last:border-0"><div><p class="font-semibold text-slate-950">{{ row.provider || 'custom' }} CSV 导入</p><p class="mt-1 text-slate-500">{{ formatDate(row.created_at) }} · 导入 {{ row.imported || 0 }} · 跳过 {{ row.skipped || 0 }}</p><ul v-if="row.errors?.length" class="mt-2 space-y-1 text-[12px] text-amber-800"><li v-for="error in row.errors.slice(0, 5)" :key="`${error.line}-${error.code}`">第 {{ error.line }} 行：{{ error.code }}</li><li v-if="row.errors.length > 5">另有 {{ row.errors.length - 5 }} 条错误（仅显示前 5 条）</li></ul></div><div class="flex items-center gap-3"><span class="rounded-full px-2.5 py-1 text-[12px] font-semibold" :class="statusClass(row.status || 'processing')">{{ statusLabel(row.status || 'processing') }}</span><a v-if="row.errors?.length" :href="`${baseUrl}/imports/${row.uuid}/errors`" class="font-semibold text-violet-700">下载失败报告</a><button v-if="canManage && row.can_undo" type="button" class="font-semibold text-red-700" @click="undoImport(row)">撤销本次导入</button></div></div></section></section>
            </template>
        ...
        <div v-if="editingReview" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4" role="presentation" @click.self="editingReview = null">
            <form role="dialog" aria-modal="true" aria-labelledby="edit-review-title" class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl" @submit.prevent="saveReview">
                <div class="flex items-center justify-between"><h2 id="edit-review-title" class="text-xl font-semibold">评价详情与审核</h2><button type="button" aria-label="关闭" class="rounded-lg p-2 text-xl" @click="editingReview = null">×</button></div>
                <dl class="mt-5 grid gap-3 rounded-xl bg-slate-50 p-4 sm:grid-cols-2">
                    <div><dt class="text-[12px] font-semibold text-slate-500">评价者</dt><dd class="mt-1 font-medium text-slate-900">{{ editingReview.author_name }}</dd></div>
                    <div><dt class="text-[12px] font-semibold text-slate-500">评价对象</dt><dd class="mt-1 font-medium text-slate-900">{{ editingReview.kind === 'product' ? (editingReview.product_title || '商品评价') : '店铺评价' }}</dd></div>
                    <div><dt class="text-[12px] font-semibold text-slate-500">提交来源</dt><dd class="mt-1 font-medium text-slate-900">{{ reviewSourceLabel(editingReview.source) }}</dd></div>
                    <div><dt class="text-[12px] font-semibold text-slate-500">真实性状态</dt><dd class="mt-1 font-medium text-slate-900">{{ verificationLabel(editingReview.verified_source) }}</dd></div>
                    <div><dt class="text-[12px] font-semibold text-slate-500">提交时间</dt><dd class="mt-1 font-medium text-slate-900">{{ formatDate(editingReview.created_at) }}</dd></div>
                    <div><dt class="text-[12px] font-semibold text-slate-500">评分</dt><dd class="mt-1 font-medium text-amber-600">{{ editingReview.rating }} / 5</dd></div>
                </dl>
                <label class="mt-5 block font-medium">状态<select v-model="editForm.status" class="mt-2 w-full rounded-xl border bg-white px-3 py-2.5"><option value="pending">待审核</option><option value="published">已发布</option><option value="unpublished">未发布</option></select></label>
                <label v-if="editForm.status === 'unpublished'" class="mt-4 block font-medium">取消发布原因<input v-model="editForm.reason" required class="mt-2 w-full rounded-xl border px-3 py-2.5" /></label>
                <label class="mt-4 flex items-center gap-2 font-medium"><input v-model="editForm.featured" type="checkbox" />设为精选评价</label>
                <p v-if="editingReview.verified_source === 'order'" class="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-800">该评价来自系统核验订单，保持“订单验证购买”，不能被人工状态覆盖。</p>
                <label v-else class="mt-4 flex items-start gap-2 rounded-xl border border-indigo-100 bg-indigo-50 p-4 font-medium text-indigo-900"><input v-model="editForm.manual_verification" type="checkbox" class="mt-1" /><span>标记为商家人工核验<small class="mt-1 block font-normal text-indigo-700">只表示商家人工确认内容或来源，不等同于系统订单验证购买，也不会获得订单验证奖励。</small></span></label>
                <label class="mt-4 block font-medium">商家回复<textarea v-model="editForm.reply" rows="5" class="mt-2 w-full rounded-xl border px-3 py-2.5"></textarea></label>
                <div v-if="Object.keys(editForm.errors).length" role="alert" class="mt-4 rounded-xl bg-red-50 p-4 text-red-700"><p v-for="(error, field) in editForm.errors" :key="field">{{ error }}</p></div>
                <div class="mt-6 flex justify-end gap-3"><button type="button" class="rounded-xl border px-5 py-2.5 font-semibold" @click="editingReview = null">取消</button><button :disabled="editForm.processing" class="rounded-xl bg-violet-700 px-5 py-2.5 font-semibold text-white disabled:opacity-50">保存</button></div>
            </form>
        </div>
