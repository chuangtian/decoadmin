...
type Settings = {
    ...
    international_delay_days: number; reminder_days: number; star_color: string; corner_style: 'rounded' | 'square';
...
const statusLabel = (status: string) => ({ pending: '待审核', published: '已发布', unpublished: '未发布', verification_required: '待验证履约与发送条件', scheduled: '已排期', sent: '已发送', unsubscribed: '已退订', cancelled: '已取消', completed: '已完成', processing: '处理中', failed: '失败', undone: '已撤销' }[status] || status);
const statusClass = (status: string) => ({ published: 'bg-emerald-50 text-emerald-700', pending: 'bg-amber-50 text-amber-700', unpublished: 'bg-slate-100 text-slate-700', failed: 'bg-red-50 text-red-700', cancelled: 'bg-slate-100 text-slate-600' }[status] || 'bg-blue-50 text-blue-700');
...
const saveSettings = () => settingsForm.put(`${props.baseUrl}/settings`, { preserveScroll: true, onSuccess: () => { notice.value = '设置已保存。'; } });
    ...
    <AppLayout :breadcrumbs="[{ label: '工作台', href: '/dashboard' }, { label: '应用中心' }, { label: 'Deco Reviews' }]">
        <div class="mx-auto max-w-7xl space-y-6 text-[14px] text-slate-800">
            ...
            <form v-else-if="tab === 'settings'" class="space-y-5" @submit.prevent="saveSettings">
                ...
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 class="text-lg font-semibold text-slate-950">邀请邮件</h2><div class="mt-5 grid gap-5 md:grid-cols-2"><label class="font-medium">回复邮箱<input v-model="settingsForm.reply_to" :disabled="!canManage" type="email" class="mt-2 w-full rounded-xl border px-3 py-2.5" /></label><label class="font-medium">主题<input v-model="settingsForm.subject" :disabled="!canManage" maxlength="160" class="mt-2 w-full rounded-xl border px-3 py-2.5" /></label><label class="font-medium md:col-span-2">正文<textarea v-model="settingsForm.email_body" :disabled="!canManage" rows="7" class="mt-2 w-full rounded-xl border px-3 py-2.5"></textarea></label></div></section>
            ...
            <template v-else-if="tab === 'widgets'">
                ...
                <section class="rounded-2xl border border-amber-200 bg-amber-50 p-5"><h2 class="font-semibold text-amber-950">集成状态</h2><p class="mt-2 text-amber-900">当前提供站内组件预览。主题应用块、外部邮件平台和第三方评价平台连接尚不可用，因此这里不会显示虚假的启用按钮。</p></section>
            </template>
        </div>
