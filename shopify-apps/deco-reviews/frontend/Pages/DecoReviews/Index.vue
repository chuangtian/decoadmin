...
type Settings = {
    enabled: boolean; organic_collection_enabled: boolean; auto_publish_days: number | null; invites_enabled: boolean; domestic_delay_days: number;
        ...
        <div class="mx-auto max-w-7xl space-y-6 text-[14px] text-slate-800">
            ...
            <form v-else-if="tab === 'settings'" class="space-y-5" @submit.prevent="saveSettings">
                <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div class="mt-5 flex flex-wrap gap-5"><label class="flex items-center gap-2"><input v-model="settingsForm.organic_collection_enabled" :disabled="!canManage" type="checkbox" />允许顾客在商品页直接评价</label><label class="flex items-center gap-2"><input v-model="settingsForm.invites_enabled" :disabled="!canManage" type="checkbox" />启用邀请排期</label><label class="flex items-center gap-2"><input v-model="settingsForm.auto_invites_enabled" :disabled="!canManage" type="checkbox" />自动识别新订单并建立邀评</label><label class="flex items-center gap-2"><input v-model="settingsForm.reminders_enabled" :disabled="!canManage" type="checkbox" />启用普通提醒</label><label class="flex items-center gap-2"><input v-model="settingsForm.media_reminders_enabled" :disabled="!canManage" type="checkbox" />启用图片/视频提醒</label><label class="flex items-center gap-2"><input v-model="settingsForm.marketing_only" :disabled="!canManage" type="checkbox" />仅邀请允许接收营销邮件的客户</label></div>
