...
const load = async () => {
    try {
        detail.value = await props.api.get<GalleryDetail>(`/galleries/${props.galleryId}`, { filter: filter.value });
...
const editProducts = (media: Media) => {
    ...
    void props.run(
        () => props.api.put<{ message: string }>(`/media/${media.id}/products`, { product_ids: productIds }),
        load,
    );
};
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            <section class="rounded-2xl border border-slate-200 bg-white p-6">
                ...
                <ul v-else class="mt-5 grid max-h-[32rem] gap-3 overflow-y-auto pr-1 sm:grid-cols-2">
                    <li v-for="media in detail.candidates" :key="media.id">
                        <label
                    ...
                    >
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="mt-2 flex flex-wrap gap-1.5">
                                <button
                                    type="button"
                                    class="rounded-lg border border-slate-300 px-2.5 py-1 text-[11px] font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50"
                                    :disabled="busy"
                                ...
                                <button
                                    ...
                                    type="button"
                                    class="rounded-lg border border-amber-300 px-2.5 py-1 text-[11px] font-semibold text-amber-700 hover:bg-amber-50 disabled:opacity-50"
                                    :disabled="busy"
                                ...
                                <button
                                    type="button"
                                    class="rounded-lg border border-rose-300 px-2.5 py-1 text-[11px] font-semibold text-rose-700 hover:bg-rose-50 disabled:opacity-50"
                                    :disabled="busy"
