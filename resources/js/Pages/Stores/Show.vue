...
<template>
    ...
    <AppLayout>
        <div class="mx-auto max-w-6xl">
            ...
            <section v-else class="mt-6 rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center"><p class="text-lg font-semibold text-slate-900">{{ tabs.find((tab) => tab.id === activeTab)?.name }}</p><p class="mt-2 text-sm text-slate-500">页面框架已准备，相关功能将在后续阶段接入。</p></section>
        </div>
    </AppLayout>
</template>
