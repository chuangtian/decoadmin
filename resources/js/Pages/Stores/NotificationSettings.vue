...
<template>
    ...
    <AppLayout :breadcrumbs="[{label:'Shopify'},{label:'店铺管理',href:'/stores'},{label:store.name,href:`/stores/${store.id}`},{label:'通知设置'}]">
        <div class="mx-auto max-w-6xl space-y-6">
            ...
            <header><p class="text-sm font-semibold text-emerald-700">店铺级通知</p><h1 class="mt-1 text-3xl font-semibold text-slate-950">{{store.name}} 通知设置</h1><p class="mt-2 text-sm text-slate-500">系统、连接和同步错误只发送站内通知；重点折扣与重点产品告警可同时推送到飞书。</p></header>
            <form class="space-y-6" @submit.prevent="submit">
                ...
                <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                    <div class="flex items-center justify-between"><div><h2 class="text-lg font-semibold">飞书机器人</h2><p class="mt-1 text-sm text-slate-500">将重点折扣与重点产品告警发送到指定飞书群；系统、连接和同步错误不发送。</p></div><label class="flex items-center gap-2 text-sm font-semibold"><input v-model="form.feishu_enabled" type="checkbox" :disabled="!canUpdate"/>启用</label></div>
