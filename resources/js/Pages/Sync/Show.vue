...
const resultLabels = {
    products: { title: '商品同步结果', eyebrow: '商品数据同步', records: '商品数量' },
    orders: { title: '订单同步结果', eyebrow: '订单数据同步', records: '订单数量' },
    customers: { title: '客户同步结果', eyebrow: '客户数据同步', records: '客户数量' },
    inventory: { title: '库存同步结果', eyebrow: '库存数据同步', records: '库存项目数量' },
} as const;
    ...
    <AppLayout :breadcrumbs="[{ label: 'Shopify' }, { label: '数据同步', href: '/sync' }, { label: syncJob.data.uuid }]">
        <div class="mx-auto max-w-6xl">
            ...
            <div class="mt-5 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div class="min-w-0"><p class="text-sm font-semibold text-emerald-700">同步任务</p><h2 class="mt-1 text-3xl font-semibold tracking-tight text-slate-950">{{ typeLabel(syncJob.data.type) }}同步</h2><p class="mt-2 break-all font-mono text-xs text-slate-400">{{ syncJob.data.uuid }}</p></div>
            ...
            <div class="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-3"><section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p class="text-xs font-semibold tracking-wider text-slate-400">店铺</p><p class="mt-3 font-semibold text-slate-900">{{ syncJob.data.store.name }}</p><p class="mt-1 break-all font-mono text-xs text-slate-400">{{ syncJob.data.store.shopify_domain }}</p></section><section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p class="text-xs font-semibold tracking-wider text-slate-400">同步模式</p><p class="mt-3 font-semibold text-slate-900">{{ modeLabel(syncJob.data.mode) }}</p><p class="mt-1 text-xs text-slate-400">{{ dateLabel(syncJob.data.since_at) }} → {{ dateLabel(syncJob.data.until_at) }}</p></section><section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p class="text-xs font-semibold tracking-wider text-slate-400">执行时间</p><p class="mt-3 text-sm font-semibold text-slate-900">{{ dateLabel(syncJob.data.started_at) }}</p><p class="mt-1 text-xs text-slate-400">完成：{{ dateLabel(syncJob.data.finished_at) }}</p></section><section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p class="text-xs font-semibold tracking-wider text-slate-400">执行次数</p><p class="mt-3 font-semibold text-slate-900">{{ syncJob.data.attempts }} / {{ syncJob.data.max_attempts }}</p></section><section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p class="text-xs font-semibold tracking-wider text-slate-400">应用安装记录</p><p class="mt-3 font-semibold text-slate-900">{{ syncJob.data.app_installation?.app?.name ?? '未指定' }}</p><p v-if="syncJob.data.app_installation" class="mt-1 text-xs text-slate-400">{{ installationStatusLabel(syncJob.data.app_installation.status) }}</p></section><section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p class="text-xs font-semibold tracking-wider text-slate-400">追踪编号</p><p class="mt-3 break-all font-mono text-xs text-slate-700">{{ syncJob.data.correlation_id ?? '—' }}</p><p v-if="syncJob.data.error_code" class="mt-2 break-all font-mono text-xs text-rose-600">{{ syncJob.data.error_code }}</p></section></div>
            ...
            <section v-if="isRealDataSync && result" class="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                <div>
                    <p class="text-sm font-semibold text-emerald-700">{{ resultLabel?.eyebrow }}</p>
                    <h3 class="mt-1 text-xl font-semibold text-slate-950">{{ resultLabel?.title }}</h3>
                </div>
                <div class="mt-5 grid gap-3 sm:grid-cols-3">
                    ...
                    <article class="rounded-xl bg-slate-50 p-4">
                        <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">{{ resultLabel?.records }}</p>
