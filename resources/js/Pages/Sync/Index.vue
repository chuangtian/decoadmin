...
const dateLabel = (value: string | null, timezone?: string) => timezone
    ? formatDateTimeInTimezone(value, timezone) : currentStoreDateTime(value);
const selectStore = () => {
                ...
                <article v-for="overview in syncStatus" :key="overview.store.id" class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div class="grid gap-px bg-slate-100 sm:grid-cols-2 xl:grid-cols-4">
                        <div v-for="state in overview.sync_states" :key="state.type" class="bg-white p-4">
                            ...
                            <dl class="mt-3 space-y-2 text-xs"><div class="flex justify-between gap-3"><dt class="text-slate-400">最近成功</dt><dd class="text-right text-slate-600">{{ dateLabel(state.last_success_at, overview.store.timezone) }}</dd></div><div class="flex justify-between gap-3"><dt class="text-slate-400">最近全量</dt><dd class="text-right text-slate-600">{{ dateLabel(state.last_full_sync_at, overview.store.timezone) }}</dd></div><div class="flex justify-between gap-3"><dt class="text-slate-400">最近增量</dt><dd class="text-right text-slate-600">{{ dateLabel(state.last_incremental_sync_at, overview.store.timezone) }}</dd></div><div class="flex justify-between gap-3"><dt class="text-slate-400">下次检查</dt><dd class="text-right text-slate-600">{{ dateLabel(state.next_sync_at, overview.store.timezone) }}</dd></div><div v-if="state.consecutive_failures" class="flex justify-between gap-3"><dt class="text-slate-400">连续失败</dt><dd class="text-right text-rose-600">{{ state.consecutive_failures }} 次</dd></div><div v-if="state.last_error_code" class="flex justify-between gap-3"><dt class="text-slate-400">错误码</dt><dd class="break-all text-right font-mono text-rose-600">{{ state.last_error_code }}</dd></div></dl>
                        </div>
                    </div>
                </article>
            </section>
