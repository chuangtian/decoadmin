    ...
    <AppLayout :breadcrumbs="[{ label: 'Shopify' }, { label: '数据同步' }]">
        <div class="mx-auto max-w-7xl">
            <div class="mb-7 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    <p class="text-sm font-semibold text-emerald-700">Sync Center</p>
                ...
                <span class="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-100">商品和订单同步已接入 Shopify，其他类型暂为任务框架</span>
            </div>
            ...
            <section class="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                <div class="mb-5">
                    ...
                    <p class="mt-1 text-sm text-slate-500">选择店铺和同步类型后，任务会进入 shopify-sync 队列。</p>
                </div>
                <form v-if="runnableStores.length" class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_180px_auto] lg:items-end" @submit.prevent="createSyncJob">
                    ...
                    <label class="block"><span class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-slate-500">App Installation</span><select v-model="createForm.app_installation_id" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-800 outline-none focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-100"><option value="">不指定 App Installation</option><option v-for="installation in selectedStore?.installations ?? []" :key="installation.id" :value="String(installation.id)">{{ installation.app?.name ?? `Installation #${installation.id}` }}</option></select><p v-if="createForm.errors.app_installation_id" class="mt-1.5 text-xs text-rose-600">{{ createForm.errors.app_installation_id }}</p></label>
                ...
                <div v-if="syncJobs.data.length">
                    <div class="hidden overflow-x-auto md:block">
                        <table class="w-full min-w-[980px] text-left text-sm">
                            <thead class="border-b border-slate-200 bg-slate-50/80 text-[11px] font-semibold uppercase tracking-wider text-slate-500"><tr><th class="px-5 py-3.5">Store</th><th class="px-5 py-3.5">Sync Type</th><th class="px-5 py-3.5">Status</th><th class="px-5 py-3.5">Started At</th><th class="px-5 py-3.5">Finished At</th><th class="px-5 py-3.5">Created At</th><th class="px-5 py-3.5 text-right">操作</th></tr></thead>
