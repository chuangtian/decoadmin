...
interface Metric { value: number; available: boolean; message: string | null; trend: number[] }
interface LiveInsight { available: boolean; message: string; items: Array<{ label: string; value: number }> }
...
interface LiveSnapshot {
    ...
    insights: { visits_by_location: LiveInsight; new_vs_returning: LiveInsight; sales_by_product: LiveInsight };
...
const initializeMap = () => {
    ...
    try {
        ...
        instance.on('style.load', () => instance.setProjection({ type: 'globe' }));
        instance.on('load', () => {
        ...
        <section ref="stageContainer" class="live-stage">
            ...
            <aside class="live-overlay">
                ...
                <section v-for="item in insightCards" :key="item.key" class="insight-card">
                    <header>
                        <h2>{{ item.title }}</h2>
                        <span v-if="!item.insight.available">暂无数据</span>
                    </header>
...
.map-topbar{position:absolute;z-index:20;top:1.25rem;right:1.25rem;left:390px;display:flex;justify-content:flex-end;gap:.75rem;pointer-events:none}.map-search,.mode-switch{pointer-events:auto;border:1px solid rgb(255 255 255/.72);background:rgb(255 255 255/.9);box-shadow:0 12px 34px rgb(15 23 42/.14);backdrop-filter:blur(18px)}.map-search{position:relative;width:min(360px,45%);border-radius:1rem}.map-search svg{position:absolute;top:50%;left:1rem;width:1.1rem;height:1.1rem;transform:translateY(-50%);color:#64748b}.map-search input{width:100%;border:0;border-radius:inherit;background:transparent;padding:.85rem 1rem .85rem 2.8rem;color:#0f172a;font-size:.875rem;outline:none}
.mode-switch{display:flex;padding:.25rem;border-radius:1rem}.mode-switch button{display:flex;align-items:center;gap:.4rem;border-radius:.75rem;padding:.6rem .75rem;color:#64748b;font-size:.78rem;font-weight:700;transition:160ms ease}.mode-switch button.active{background:#0f172a;color:#fff;box-shadow:0 6px 14px rgb(15 23 42/.2)}.mode-switch svg{width:1.05rem;height:1.05rem}
.live-overlay{position:absolute;z-index:25;top:1rem;bottom:1rem;left:1rem;display:flex;width:350px;flex-direction:column;overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain;scrollbar-width:thin;scrollbar-color:rgb(148 163 184/.55) transparent;border:1px solid rgb(255 255 255/.78);border-radius:1.6rem;background:rgb(255 255 255/.91);box-shadow:0 24px 60px rgb(15 23 42/.2);backdrop-filter:blur(22px) saturate(135%)}.live-overlay::-webkit-scrollbar{width:6px}.live-overlay::-webkit-scrollbar-track{background:transparent}.live-overlay::-webkit-scrollbar-thumb{border-radius:999px;background:rgb(148 163 184/.55)}.overlay-header{display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;padding:1.25rem 1.25rem 1rem}.live-status{display:flex;align-items:center;gap:.45rem;color:#047857;font-size:.68rem;font-weight:800;letter-spacing:.13em}.live-status span{width:.48rem;height:.48rem;border-radius:999px;background:#10b981;box-shadow:0 0 0 5px rgb(16 185 129/.13);animation:pulse 2s ease-in-out infinite}.overlay-header h1{margin-top:.55rem;color:#0f172a;font-size:1.55rem;font-weight:750;letter-spacing:-.035em}.overlay-header p{margin-top:.25rem;color:#64748b;font-size:.76rem}.refresh-button{display:flex;align-items:center;gap:.35rem;border:1px solid #e2e8f0;border-radius:.75rem;background:rgb(248 250 252/.9);padding:.52rem .62rem;color:#475569;font-size:.68rem;font-weight:700}.refresh-button svg{width:.85rem;height:.85rem}.refresh-button svg.spinning{animation:spin 1s linear infinite}
.metric-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.65rem;padding:0 1rem 1rem}.metric-card{min-height:112px;border:1px solid rgb(226 232 240/.85);border-radius:1.1rem;background:rgb(248 250 252/.78);padding:.85rem}.metric-label{display:flex;align-items:center;justify-content:space-between;gap:.4rem;color:#64748b;font-size:.72rem;font-weight:650}.metric-label em{border-radius:999px;background:#fef3c7;padding:.15rem .38rem;color:#a16207;font-size:.55rem;font-style:normal}.metric-card strong{display:block;margin-top:.55rem;overflow:hidden;color:#0f172a;font-size:1.35rem;font-weight:760;letter-spacing:-.035em;text-overflow:ellipsis;white-space:nowrap}.metric-card svg{width:100%;height:1.5rem;margin-top:.4rem;color:#10b981}.metric-card small{display:-webkit-box;margin-top:.45rem;overflow:hidden;color:#94a3b8;font-size:.62rem;line-height:1.35;-webkit-box-orient:vertical;-webkit-line-clamp:2}
.overlay-section{margin:0 1rem .75rem;border:1px solid rgb(226 232 240/.82);border-radius:1.1rem;background:rgb(255 255 255/.7);padding:.9rem}.section-heading{display:flex;align-items:center;justify-content:space-between;gap:.75rem}.section-heading>div:first-child{display:flex;flex-direction:column}.section-heading span{color:#1e293b;font-size:.75rem;font-weight:750}.section-heading small{margin-top:.1rem;color:#94a3b8;font-size:.62rem}.connection-pill{border-radius:999px;background:#f1f5f9;padding:.3rem .55rem;color:#64748b!important;font-size:.58rem!important}.connection-pill.connected{background:#d1fae5;color:#047857!important}.behavior-row{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));margin-top:.75rem}.behavior-row div{text-align:center}.behavior-row div+div{border-left:1px solid #e2e8f0}.behavior-row strong,.behavior-row span{display:block}.behavior-row strong{color:#0f172a;font-size:1.05rem}.behavior-row span{margin-top:.18rem;color:#64748b;font-size:.6rem}
.insight-card{flex:none;min-height:150px;margin:0 1rem .75rem;border:1px solid rgb(226 232 240/.82);border-radius:1.1rem;background:rgb(255 255 255/.76);padding:.95rem}.insight-card header{display:flex;align-items:center;justify-content:space-between;gap:.75rem}.insight-card h2{color:#1e293b;font-size:.78rem;font-weight:750}.insight-card header>span{border-radius:999px;background:#f1f5f9;padding:.25rem .5rem;color:#94a3b8;font-size:.56rem;font-weight:700}.insight-empty{display:grid;min-height:100px;place-items:center;align-content:center;gap:.45rem;color:#94a3b8;text-align:center}.insight-empty svg{width:1.35rem;height:1.35rem}.insight-empty p{font-size:.66rem}.insight-list{margin-top:.7rem}.insight-list div{display:flex;align-items:center;justify-content:space-between;border-top:1px solid #f1f5f9;padding:.55rem .1rem;color:#64748b;font-size:.68rem}.insight-list strong{color:#0f172a}
.location-section{flex:none;overflow:hidden}.layer-switch{display:flex!important;flex-direction:row!important;border-radius:.6rem;background:#f1f5f9;padding:.18rem}.layer-switch button{border-radius:.45rem;padding:.28rem .48rem;color:#64748b;font-size:.58rem;font-weight:700}.layer-switch button.active{background:#fff;color:#0f172a;box-shadow:0 2px 6px rgb(15 23 42/.1)}.location-list{max-height:180px;margin-top:.65rem;overflow-y:auto}.location-list button{display:flex;width:100%;align-items:center;justify-content:space-between;gap:1rem;border-top:1px solid #f1f5f9;padding:.62rem .2rem;color:#475569;font-size:.68rem;text-align:left}.location-list button span{display:flex;align-items:center;gap:.45rem}.location-list strong{color:#0f172a}.location-list i,.map-legend i{width:.45rem;height:.45rem;border-radius:999px;background:#8b5cf6;box-shadow:0 0 0 4px rgb(139 92 246/.14)}.location-list i.visitors,.map-legend i.visitors{background:#0ea5e9;box-shadow:0 0 0 4px rgb(14 165 233/.14)}
.empty-location{display:grid;min-height:105px;place-items:center;padding:.75rem;color:#94a3b8;text-align:center}.empty-location svg{width:1.5rem;height:1.5rem}.empty-location p{max-width:230px;margin-top:.4rem;font-size:.64rem;line-height:1.5}.privacy-note{display:flex;align-items:center;gap:.4rem;padding:0 1.25rem 1rem;color:#94a3b8;font-size:.58rem}.privacy-note svg{width:.8rem;height:.8rem}
.map-legend{position:absolute;z-index:15;right:5rem;bottom:1.25rem;display:flex;align-items:center;gap:.7rem;border:1px solid rgb(255 255 255/.72);border-radius:.9rem;background:rgb(255 255 255/.88);padding:.65rem .85rem;box-shadow:0 10px 28px rgb(15 23 42/.14);backdrop-filter:blur(16px)}.map-legend span{display:flex;align-items:center;gap:.45rem;color:#334155;font-size:.66rem;font-weight:750}.map-legend small{border-left:1px solid #e2e8f0;padding-left:.7rem;color:#64748b;font-size:.6rem}
.map-state{position:absolute;z-index:40;inset:0;display:grid;place-items:center;background:#dceff1}.map-state>div{border:1px solid rgb(255 255 255/.8);border-radius:1rem;background:rgb(255 255 255/.9);padding:.8rem 1rem;color:#475569;font-size:.75rem;font-weight:700;box-shadow:0 12px 35px rgb(15 23 42/.12)}.map-state span{display:inline-block;width:.55rem;height:.55rem;margin-right:.5rem;border-radius:999px;background:#10b981;animation:pulse 1.6s ease-in-out infinite}.map-state--error>div{max-width:360px;padding:1.4rem;text-align:center}.map-state--error strong{color:#0f172a}.map-state--error p{margin-top:.45rem;color:#64748b;font-size:.72rem;font-weight:400}.map-state--error button{margin-top:.9rem;border-radius:.7rem;background:#0f172a;padding:.55rem .8rem;color:#fff;font-size:.68rem}
:deep(.maplibregl-ctrl-bottom-right){right:.8rem;bottom:.8rem}:deep(.maplibregl-ctrl-group){overflow:hidden;border:1px solid rgb(255 255 255/.8);border-radius:.85rem;background:rgb(255 255 255/.9);box-shadow:0 10px 28px rgb(15 23 42/.14);backdrop-filter:blur(16px)}:deep(.maplibregl-ctrl-group button){width:36px;height:36px}:deep(.maplibregl-popup-content){border-radius:.8rem;padding:.8rem .9rem;box-shadow:0 12px 35px rgb(15 23 42/.18)}:deep(.live-popup){display:grid;gap:.2rem;color:#0f172a;font-size:.72rem}:deep(.live-popup span){color:#64748b}
...
@media(max-width:640px){.live-stage{border-radius:1.3rem}.map-topbar{top:.75rem;right:.75rem;left:.75rem}.map-search{width:auto;flex:1}.mode-switch span{display:none}.live-overlay{right:.75rem;bottom:.75rem;left:.75rem;max-height:51%;border-radius:1.25rem}.overlay-header h1{font-size:1.25rem}.refresh-button span{display:none}.metric-grid{gap:.45rem;padding-right:.75rem;padding-left:.75rem}.metric-card{min-height:80px;padding:.7rem}.metric-card strong{font-size:1.08rem}.overlay-section{margin-right:.75rem;margin-left:.75rem}}
