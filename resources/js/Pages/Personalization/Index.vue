...
const props = defineProps<{
    ...
    checkout: { uuid: string | null; strategy_uuid: string | null; trust_items: CheckoutTrustItem[]; icon_options: Array<{ value: string; label: string }> };
...
function saveCheckout() { checkoutForm.transform(data => ({ strategy_uuid: data.strategy_uuid, trust_items: data.trust_items.map((item, index) => ({ ...item, position: index + 1 })) })).put(`${baseUrl}/checkout`, { preserveScroll: true }); }
