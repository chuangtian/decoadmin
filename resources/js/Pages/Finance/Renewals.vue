...
const renewalCount = computed(() => props.paymentRequests.filter((item) => item.payment_status === 'paid').length);
    ...
    <AppLayout :breadcrumbs="[{ label: '公司财务' }, { label: '续费' }]">
        <main class="mx-auto w-full max-w-7xl space-y-6">
            ...
            <section class="grid gap-4 sm:grid-cols-3">
                <article class="rounded-2xl border border-amber-100 bg-amber-50 p-5"><p class="text-sm font-semibold text-amber-700">待付款</p><p class="mt-3 text-3xl font-bold text-amber-950">{{ pendingCount }}</p></article>
                <article class="rounded-2xl border border-blue-100 bg-blue-50 p-5"><p class="text-sm font-semibold text-blue-700">持续续费项目</p><p class="mt-3 text-3xl font-bold text-blue-950">{{ renewalCount }}</p></article>
            ...
            <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-100 px-6 py-5"><h2 class="font-semibold text-slate-900">付款与续费项目</h2><p class="mt-1 text-sm text-slate-500">待付款优先显示，已付款的软件按下一续费日期排列。</p></div>
                <p v-if="revealError" class="mx-6 mt-4 rounded-xl bg-rose-50 p-3 text-sm text-rose-700">{{ revealError }}</p>
                <div v-if="paymentRequests.length" class="divide-y divide-slate-100">
                    <article v-for="item in paymentRequests" :key="item.uuid" class="grid gap-5 px-6 py-5 lg:grid-cols-[minmax(0,1fr)_220px_auto] lg:items-center" :class="item.is_new_application ? 'bg-rose-50' : item.next_renewal_on && item.next_renewal_on < today ? 'bg-rose-50/50' : ''">
                        <div class="min-w-0">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <span class="rounded-full px-2.5 py-1 text-xs font-semibold" :class="item.payment_status === 'pending' ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'">{{ item.payment_status === 'pending' ? '待付款' : '续费中' }}</span>
                            </div>
                            ...
                            <div v-if="item.category === 'software'" class="mt-3 flex flex-wrap gap-x-4 gap-y-2 rounded-xl bg-slate-50 p-3 text-xs text-slate-600">
                                <a :href="item.software_url || '#'" target="_blank" rel="noreferrer" class="font-semibold text-blue-600 hover:underline">打开软件网站</a>
                                <span class="inline-flex items-center gap-1">账号：<strong class="font-mono text-slate-800">{{ item.software_account }}</strong><button v-if="canManage && item.software_account" type="button" class="font-semibold text-blue-600 hover:underline" @click="copyValue(item.software_account || '', `${item.uuid}-account`)">{{ copied === `${item.uuid}-account` ? '已复制' : '复制' }}</button></span>
                                ...
                                <span>{{ item.renewal_mode === 'automatic' ? '自动续费' : '手动续费' }} · {{ item.billing_cycle === 'monthly' ? '月付' : '年付' }}</span>
                            </div>
                        </div>
                        ...
                        <button v-if="canManage && (item.payment_status === 'pending' || item.renewal_mode === 'manual')" class="h-11 rounded-xl bg-slate-950 px-5 text-sm font-semibold text-white" @click="openPayment(item)">{{ item.payment_status === 'pending' ? '确认付款' : '记录续费' }}</button>
                        <span v-else-if="item.payment_status === 'paid' && item.renewal_mode === 'automatic'" class="rounded-xl bg-blue-50 px-4 py-3 text-center text-xs font-semibold text-blue-700">到期自动记为已付款</span>
                    </article>
                </div>
                <EmptyState v-else title="暂无付款或续费项目" description="费用申请审批通过后会自动进入这里。" icon="finance" />
            </section>
            ...
            <dialog :open="selected !== null" class="fixed inset-0 z-50 m-0 h-full w-full max-w-none bg-slate-950/45 p-4" @click.self="selected = null">
                <form v-if="selected" class="mx-auto mt-[14vh] max-w-lg rounded-3xl bg-white p-7 shadow-2xl" @submit.prevent="savePayment">
                    <div class="flex items-start justify-between gap-4">
                        <div><p class="text-xs font-semibold text-emerald-700">{{ selected.reference_no }}</p><h2 class="mt-1 text-xl font-bold text-slate-950">{{ selected.payment_status === 'pending' ? '确认付款' : '记录本次续费' }}</h2><p class="mt-1 text-sm text-slate-500">{{ selected.title }} · {{ selected.currency }} {{ selected.amount }}</p></div>
                        <button type="button" class="text-2xl text-slate-400" @click="selected = null">×</button>
                    </div>
                    <div v-if="selected.category === 'software'" class="mt-5 rounded-xl bg-orange-50 p-4 text-sm text-orange-900"><p>{{ selected.renewal_mode === 'automatic' ? '自动续费' : '手动续费' }} · {{ selected.billing_cycle === 'monthly' ? '月付' : '年付' }}</p><p class="mt-1 text-xs text-orange-700">确认后系统会从本次付款日期起计算下一次续费日期。</p></div>
