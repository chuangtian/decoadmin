...
interface AppLog { id: number; uuid: string; action: string; action_label: string; store: { id: number; name: string } | null; actor: string; created_at: string | null }
...
<template>
    ...
    <AppLayout :breadcrumbs="[{ label: '应用中心' }, { label: '应用日志' }]">
        <div class="mx-auto max-w-7xl">
            <header><p class="text-sm font-semibold text-emerald-700">App Center</p><h1 class="mt-1 text-3xl font-semibold tracking-tight text-slate-950">应用日志</h1><p class="mt-2 text-sm text-slate-500">集中查看应用安装、授权和凭证变更记录。</p></header>
