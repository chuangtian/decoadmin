...
const currentInstallationStatus = (app: ShopifyApp) => app.current_store_installation?.status ?? 'not_installed';
</script>

<template>
    <Head title="应用中心" />
    <AppLayout :breadcrumbs="[{ label: '应用中心' }]">
        <div class="mx-auto max-w-7xl">
            <div class="mb-7">
                <p class="text-sm font-semibold text-emerald-700">应用中心</p>
                <h2 class="mt-1 text-3xl font-semibold tracking-tight text-slate-950">应用中心</h2>
                <p class="mt-2 text-sm text-slate-500">查看当前组织可用的 Shopify 应用，以及右上角当前店铺的安装状态。</p>
            </div>
                ...
                <div v-if="apps.data.length">
                        ...
                        <table class="w-full min-w-[880px] text-left text-sm">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <tr v-for="app in apps.data" :key="app.id" class="transition hover:bg-slate-50/70">
                                    <td class="px-5 py-4"><div class="flex items-center gap-3"><span class="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-emerald-50 text-sm font-bold text-emerald-700">{{ app.name.charAt(0).toUpperCase() }}</span><div class="min-w-0"><Link :href="`/app-center/${app.id}`" class="font-semibold text-slate-900 hover:text-emerald-700">{{ app.name }}</Link><p class="mt-0.5 font-mono text-xs text-slate-400">{{ app.slug }}</p></div></div></td>
                                    ...
                                    <td class="px-5 py-4"><span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ring-1" :class="installationStatusClass(currentInstallationStatus(app))">{{ installationStatusLabel(currentInstallationStatus(app)) }}</span></td>
                                    <td class="px-5 py-4 text-xs text-slate-500">{{ dateLabel(app.created_at) }}</td>
                                    <td class="px-5 py-4 text-right"><Link :href="`/app-center/${app.id}`" class="rounded-lg px-3 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-50">查看详情</Link></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="divide-y divide-slate-100 md:hidden">
                        <article v-for="app in apps.data" :key="app.id" class="p-5">
                            <div class="flex items-start justify-between gap-3"><div class="flex min-w-0 items-center gap-3"><span class="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-emerald-50 text-sm font-bold text-emerald-700">{{ app.name.charAt(0).toUpperCase() }}</span><div class="min-w-0"><Link :href="`/app-center/${app.id}`" class="block truncate font-semibold text-slate-900">{{ app.name }}</Link><p class="mt-0.5 truncate font-mono text-xs text-slate-400">{{ app.slug }}</p></div></div><span class="inline-flex shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ring-1" :class="statusClass(app.status)">{{ statusLabel(app.status) }}</span></div>
                            <dl class="mt-4 grid grid-cols-2 gap-3 rounded-xl bg-slate-50 p-3 text-sm"><div><dt class="text-xs text-slate-400">类型</dt><dd class="mt-1 font-medium text-slate-700">{{ typeLabel(app.type) }}</dd></div><div><dt class="text-xs text-slate-400">当前店铺安装</dt><dd class="mt-1"><span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ring-1" :class="installationStatusClass(currentInstallationStatus(app))">{{ installationStatusLabel(currentInstallationStatus(app)) }}</span></dd></div></dl>
                            <Link :href="`/app-center/${app.id}`" class="mt-4 inline-flex text-sm font-semibold text-emerald-700">查看详情 →</Link>
                        </article>
                    </div>
                </div>
