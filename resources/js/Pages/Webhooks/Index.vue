...
<template>
    ...
    <AppLayout :breadcrumbs="[{ label: 'Shopify' }, { label: 'Webhook Events' }]">
        <div class="mx-auto max-w-7xl">
            <div class="mb-7">
                <p class="text-sm font-semibold text-emerald-700">Webhook Center</p>
                <h2 class="mt-1 text-3xl font-semibold tracking-tight text-slate-950">Webhook Events</h2>
            ...
            <form class="mb-5 grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:grid-cols-[minmax(0,1fr)_180px_220px_auto]" @submit.prevent="search">
                <label class="block"><span class="sr-only">搜索 Event Type</span><input v-model="form.topic" type="search" placeholder="搜索 Event Type，例如 orders/create" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-100" /></label>
