...
const dateLabel = (value: string | null) => value
    ...
    : '—';
const statusLabel = (status: string) => ({ pending: '等待处理', processing: '处理中', processed: '已处理', failed: '处理失败' }[status] ?? status);
const statusClass = (status: string) => ({ pending: 'bg-amber-50 text-amber-700 ring-amber-100', processing: 'bg-blue-50 text-blue-700 ring-blue-100', processed: 'bg-emerald-50 text-emerald-700 ring-emerald-100', failed: 'bg-rose-50 text-rose-700 ring-rose-100' }[status] ?? 'bg-slate-50 text-slate-600 ring-slate-200');
const retry = () => {
