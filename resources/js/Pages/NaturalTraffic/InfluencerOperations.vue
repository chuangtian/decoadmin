...
const rawColumns = computed(() => props.dashboard.columns);
...
<template>
    ...
    <AppLayout :breadcrumbs="[{ label: '自然流量' }, { label: '红人运营' }]">
        <div class="mx-auto w-full max-w-[1680px] space-y-4">
            <NaturalTrafficPageHeader :dashboard="dashboard" :store="store" :configured="configured" :can-sync="canSync" route-path="/natural-traffic/influencer-operations" refresh-path="/natural-traffic/influencer-operations/refresh" :active-tab="activeTab" accent="violet" compact @tab="activeTab = $event" />
            ...
            <template v-else>
                ...
                <template v-if="activeTab === 'tracking'">
                    ...
                    <div class="grid gap-4" :class="dashboard.model_summary.length ? 'xl:grid-cols-[minmax(0,1.25fr)_minmax(360px,.75fr)]' : ''">
                        <section class="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm"><h2 class="mb-3 text-base font-black text-slate-950">合作数据明细</h2><NaturalTrafficDataTable :columns="detailColumns" :rows="detailRows" :labels="labels" :page-size="8" compact /></section>
                    ...
                    <section v-if="dashboard.viral_content.length" class="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm"><h2 class="text-base font-black text-slate-950">爆款内容统计</h2><p class="mt-0.5 mb-3 text-xs text-slate-500">独立爆款统计表中的非 AI 数据，按发布日期从新到旧</p><NaturalTrafficDataTable :columns="detailColumns" :rows="dashboard.viral_content" :labels="labels" :page-size="8" compact /></section>
                    <section class="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm"><div class="mb-3"><h2 class="text-base font-black text-slate-950">筛选期完整源字段</h2><p class="mt-0.5 text-xs text-slate-500">表头按原表字段顺序展示，记录按发布日期从新到旧</p></div><NaturalTrafficDataTable :columns="rawColumns" :rows="detailRows" :labels="{}" :page-size="8" compact /></section>
                </template>
                <template v-else>
                    <section class="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm"><div class="mb-3"><h2 class="text-base font-black text-slate-950">红人资源库</h2><p class="mt-0.5 text-xs text-slate-500">按红人和平台聚合历史合作，按最近合作日期从新到旧</p></div><NaturalTrafficDataTable :columns="['influencer', 'platform', 'type', 'date', 'collaborations', 'average_views', 'fee', 'engagement_rate', 'views', 'link']" :rows="dashboard.resources" :labels="{ influencer: '红人', platform: '平台', type: '类型', date: '最近合作', collaborations: '合作次数', average_views: '均播', fee: '费用', engagement_rate: '互动率 %', views: '累计浏览', link: '链接' }" :page-size="10" compact /></section>
                    <section class="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm"><div class="mb-3"><h2 class="text-base font-black text-slate-950">筛选期合作资源明细</h2><p class="mt-0.5 text-xs text-slate-500">保留本期逐条合作、费用、表现与链接，按发布日期从新到旧</p></div><NaturalTrafficDataTable :columns="detailColumns" :rows="detailRows" :labels="labels" :page-size="10" compact /></section>
                </template>
            </template>
        </div>
    </AppLayout>
