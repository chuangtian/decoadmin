        ...
        <div class="mx-auto w-full max-w-[1680px] space-y-4">
            ...
            <template v-else>
                ...
                <div class="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(420px,.9fr)]">
                    ...
                    <section class="rounded-[22px] border border-slate-200 bg-white p-4 shadow-sm"><h2 class="text-base font-black text-slate-950">红人浏览 TOP 10</h2><p class="mt-0.5 text-xs text-slate-500">同一红人合并统计</p><div class="mt-3 grid gap-x-5 gap-y-2 2xl:grid-cols-2"><div v-for="(row, index) in dashboard.top_influencers" :key="String(row.name)" class="grid grid-cols-[22px_minmax(0,1fr)_auto] items-center gap-2"><span class="text-xs font-black text-slate-400">{{ index + 1 }}</span><div><div class="flex justify-between gap-2 text-xs"><strong class="truncate text-slate-800">{{ row.name }}</strong><span class="text-xs text-slate-400">{{ row.platform }}</span></div><div class="mt-1 h-1.5 overflow-hidden rounded-full bg-slate-100"><div class="h-full rounded-full bg-violet-500" :style="{ width: `${Math.max(2, Number(row.views ?? 0) / topMax * 100)}%` }"></div></div></div><strong class="text-xs tabular-nums text-slate-800">{{ compact(row.views) }}</strong></div><p v-if="!dashboard.top_influencers.length" class="py-8 text-center text-sm text-slate-400">暂无排行数据</p></div></section>
                </div>
