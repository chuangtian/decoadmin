                ...
                <template v-if="activeTab === 'overview'">
                    ...
                    <div class="grid gap-6 xl:grid-cols-2">
                        <section class="rounded-[26px] border border-slate-200 bg-white p-6 shadow-sm">
                            ...
                            <div class="space-y-4"><article v-for="(flow, index) in dashboard.top_flows" :key="String(flow.name)" class="rounded-2xl bg-slate-50 p-4"><div class="flex items-center justify-between gap-4"><div class="min-w-0"><p class="truncate text-sm font-black text-slate-900">{{ index + 1 }}. {{ flow.name }}</p><p class="mt-1 text-xs text-slate-500">{{ flow.records }} 条记录 · 打开率 {{ Number(flow.open_rate ?? 0).toFixed(2) }}%</p></div><strong class="whitespace-nowrap tabular-nums text-emerald-700">{{ money(flow.revenue) }}</strong></div><div class="mt-3 h-2 overflow-hidden rounded-full bg-slate-200"><div class="h-full rounded-full bg-emerald-500" :style="{ width: `${Math.min(100, Number(flow.revenue_share ?? 0))}%` }"></div></div><p class="mt-1.5 text-right text-xs font-bold text-slate-400">贡献 {{ Number(flow.revenue_share ?? 0).toFixed(2) }}%</p></article><p v-if="!dashboard.top_flows.length" class="py-12 text-center text-sm text-slate-400">当前筛选暂无序列收入</p></div>
                        </section>
                        <section class="rounded-[26px] border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <template v-else>
                    <div v-if="dashboard.target_progress.length" class="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
                        <article v-for="item in dashboard.target_progress" :key="item.key" class="rounded-[22px] border border-slate-200 bg-white p-5 shadow-sm">
                            <div class="flex items-start justify-between gap-2"><p class="text-xs font-black text-slate-600">{{ item.label }}</p><span class="rounded-full px-2.5 py-1 text-xs font-black" :class="targetStatusClass(item)">{{ targetStatus(item) }}</span></div>
                            ...
                            <div class="mt-4 h-2 overflow-hidden rounded-full bg-slate-100"><div class="h-full rounded-full bg-emerald-500" :style="{ width: `${Math.min(100, Math.max(0, item.progress ?? 0))}%` }"></div></div><p class="mt-1.5 text-right text-xs font-bold text-slate-400">{{ item.progress === null ? '等待源表填写' : `${item.progress.toFixed(1)}%` }}</p>
                        </article>
                    </div>
                    <section class="rounded-[26px] border border-slate-200 bg-white p-6 shadow-sm">
