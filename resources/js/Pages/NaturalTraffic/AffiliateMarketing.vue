        ...
        <div class="mx-auto w-full max-w-[1680px] space-y-6">
            ...
            <template v-else>
                ...
                <div class="grid gap-6 xl:grid-cols-2">
                    <section class="rounded-[26px] border border-slate-200 bg-white p-6 shadow-sm"><h2 class="text-lg font-black text-slate-950">GMV 趋势（按周）</h2><p class="mt-1 mb-5 text-xs text-slate-500">同一周的不同联盟自动合并</p><NaturalTrafficTrendChart :points="dashboard.weekly" x-key="week" :series="[{ key: 'gmv', label: 'GMV', color: '#f59e0b' }]" /></section>
                    <section class="rounded-[26px] border border-slate-200 bg-white p-6 shadow-sm"><h2 class="text-lg font-black text-slate-950">Clicks 与 GMV 趋势</h2><p class="mt-1 mb-5 text-xs text-slate-500">点击规模与产出效率同步观察</p><NaturalTrafficTrendChart :points="dashboard.weekly" x-key="week" :series="[{ key: 'clicks', label: 'Clicks', color: '#10b981' }, { key: 'gmv', label: 'GMV', color: '#7c3aed' }]" /></section>
                </div>
