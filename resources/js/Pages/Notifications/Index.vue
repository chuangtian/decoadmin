...
const submit = () =>
    router.get(
        "/notifications",
        { status: form.status || undefined, type: form.type || undefined },
        { preserveState: true },
    );
const resend = (id: number) =>
    router.post(`/notifications/${id}/resend`, {}, { preserveScroll: true });
        ...
        ><div class="mx-auto max-w-7xl space-y-6">
            ...
            >
                ...
                >
                    ...
                    <option value="skipped">未启用</option></select
                ><select
                    v-model="form.type"
                    class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm"
                >
                    ...
                    <option value="connection">连接状态</option></select
                ><button
            ...
            <section v-if="notifications.data.length" class="space-y-3">
                <article
                ...
                >
                    ...
                    >
                        <div class="min-w-0">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                ><span
                                    ...
                                    :class="statusClass(item.delivery_status)"
                                    >{{
                            ...
                            <p class="mt-3 text-xs text-slate-400">
                                {{ date(item.occurred_at) }} · {{ item.code }} ·
                                已尝试 {{ item.delivery_attempts }} 次
                            </p>
                            <p
                                v-if="item.delivery_error"
                        ...
                        <button
                            ...
                            "
                            class="shrink-0 rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:border-emerald-300 hover:text-emerald-700"
            ...
            <EmptyState
                v-else
                title="暂无通知记录"
                description="发生连接、同步或 Webhook 异常后，通知记录会显示在这里。"
                icon="status"
            /><Pagination :links="notifications.links" /></div
    ></AppLayout>
</template>
