...
const props = withDefaults(defineProps<{
    ...
    emptyText?: string;
    compact?: boolean;
}>(), { labels: () => ({}), pageSize: 20, emptyText: '当前筛选暂无数据', compact: false });
const page = ref(1);
...
<template>
    <div>
        <div class="overflow-x-auto border border-slate-200" :class="compact ? 'rounded-xl' : 'rounded-2xl'">
            <table class="w-full min-w-max text-left" :class="compact ? 'text-xs' : 'text-sm'">
                <thead class="bg-slate-50 text-[11px] font-black uppercase tracking-wider text-slate-500"><tr><th v-for="column in columns" :key="column" class="whitespace-nowrap" :class="compact ? 'px-3 py-2.5' : 'px-4 py-3.5'">{{ labels[column] || column }}</th></tr></thead>
                <tbody class="divide-y divide-slate-100 bg-white">
                    ...
                    <tr v-if="!pageRows.length"><td :colspan="Math.max(1, columns.length)" class="px-5 text-center text-sm font-semibold text-slate-400" :class="compact ? 'py-8' : 'py-14'">{{ emptyText }}</td></tr>
                </tbody>
            </table>
        </div>
        <div v-if="rows.length > pageSize" class="flex items-center justify-between text-xs font-semibold text-slate-500" :class="compact ? 'mt-2' : 'mt-3'">
