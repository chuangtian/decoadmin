...
const width = 760;
const padding = { left: 88, right: 36, top: 32, bottom: 64 };
...
<template>
    <div v-if="points.length" class="overflow-hidden">
        ...
        <svg v-readable-chart :viewBox="`0 0 ${width} ${height}`" class="block min-w-[640px] w-full" role="img" :aria-label="`${xLabel}与${yLabel}散点图`">
            <g v-for="tick in [0, .25, .5, .75, 1]" :key="`y-${tick}`">
                ...
                <text :x="padding.left - 10" :y="padding.top + plotHeight * (1 - tick) + 4" text-anchor="end" fill="#94a3b8" font-size="12">{{ compact(maxRate * tick) }}</text>
            </g>
            <g v-for="tick in [0, .25, .5, .75, 1]" :key="`x-${tick}`">
                <text :x="padding.left + plotWidth * tick" :y="height - 30" text-anchor="middle" fill="#94a3b8" font-size="12">{{ compact(xTick(tick)) }}</text>
            </g>
            ...
            <text :x="width / 2" :y="height - 8" text-anchor="middle" fill="#64748b" font-size="12">{{ xLabel }}</text>
        </svg>
        </div>
    </div>
