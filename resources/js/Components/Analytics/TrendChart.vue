...
<template>
    <div class="flex h-56 items-end gap-1.5 sm:gap-2">
        <div v-for="(point, index) in points" :key="`${metric}-${point.label}-${index}`" class="group flex min-w-0 flex-1 flex-col items-center justify-end gap-2">
            <div class="invisible absolute -translate-y-7 rounded-lg bg-slate-950 px-2 py-1 text-[10px] font-semibold text-white group-hover:visible">
            ...
            <span class="h-4 truncate text-[10px] text-slate-400">{{ index % visibleLabels === 0 || index === points.length - 1 ? point.label : '' }}</span>
        </div>
    </div>
</template>
