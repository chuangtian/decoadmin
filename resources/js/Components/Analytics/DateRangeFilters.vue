...
const apply = () => router.get(props.action, { ...form, ...(props.extra ?? {}) }, {
    preserveState: true,
    preserveScroll: true,
    replace: true,
});
    ...
    <form class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm" @submit.prevent="apply">
        <div class="flex flex-col gap-4 xl:flex-row xl:items-end">
            <div class="flex flex-wrap gap-2">
                <button v-for="days in [7, 30, 90]" :key="days" type="button" class="rounded-xl px-3.5 py-2 text-sm font-semibold transition" :class="form.days === days && !form.date_from ? 'bg-slate-950 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'" @click="preset(days)">
                    {{ days }} 天
                </button>
            </div>
            <label class="min-w-0 flex-1 text-xs font-semibold text-slate-500">开始日期
                <input v-model="form.date_from" type="date" class="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm text-slate-800" required>
            </label>
            <label class="min-w-0 flex-1 text-xs font-semibold text-slate-500">结束日期
                <input v-model="form.date_to" type="date" class="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm text-slate-800" required>
            </label>
