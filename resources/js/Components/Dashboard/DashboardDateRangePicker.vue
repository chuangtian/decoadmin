...
const canClearSelection = computed(() => selectionTarget.value === 'comparison'
    ...
    : Boolean(rangeStart.value || rangeEnd.value));
...
function selectCurrentRange() {
    selectionTarget.value = 'current';
    viewMonth.value = startOfMonth(parseDate(rangeStart.value || props.period.from || toDate(today)));
}

function selectComparisonRange() {
    selectionTarget.value = 'comparison';
    if (comparisonStart.value) viewMonth.value = startOfMonth(parseDate(comparisonStart.value));
}
            ...
            <div class="grid lg:grid-cols-[220px_minmax(0,1fr)]">
                ...
                <div class="p-4 sm:p-5">
                    ...
                    <div class="mt-4 flex items-center justify-end gap-3 border-t border-slate-100 pt-3">
                        <button type="button" class="rounded-xl border border-rose-200 px-5 py-2.5 text-sm font-semibold text-rose-600 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-40" :disabled="!canClearSelection" @click="clearSelection">清除</button>
