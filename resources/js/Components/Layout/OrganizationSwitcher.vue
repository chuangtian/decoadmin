...
<template>
    <details ref="details" class="group relative min-w-0">
        <summary class="flex h-11 cursor-pointer list-none items-center gap-2 rounded-xl px-2.5 transition hover:bg-slate-100">
            <span class="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-slate-900 text-xs font-bold text-white">{{ page.props.currentOrganization?.name.charAt(0).toUpperCase() ?? 'O' }}</span>
            <span class="hidden min-w-0 text-left md:block"><span class="block text-[9px] font-semibold uppercase tracking-[0.16em] text-slate-400">当前组织</span><span class="block max-w-28 truncate text-sm font-semibold text-slate-800">{{ page.props.currentOrganization?.name ?? '暂无组织' }}</span></span>
        ...
        <div class="absolute left-0 z-40 mt-2 w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white p-2 shadow-xl shadow-slate-900/10">
