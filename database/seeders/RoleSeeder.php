...
class RoleSeeder extends Seeder
{
    ...
    private array $roles = [
        ...
        'organization-admin' => [
            ...
            'permissions' => ['organization.*', 'store.*', 'users.*', 'roles.*', 'apps.*', 'webhooks.*', 'sync.*', 'alerts.*', 'finance.*', 'reports.*', 'student_discount.*', 'instagram_feed.*', 'products.sync', 'orders.sync', 'customers.sync', 'inventory.sync', '*.view'],
        ],
        'store-admin' => [
            'name' => '店铺管理员',
            'description' => '管理指定店铺及其日常业务运营。',
            'permissions' => ['store.view', 'store.update', 'store.connect', 'store.disconnect', 'orders.*', 'products.*', 'customers.*', 'inventory.*', 'webhooks.*', 'sync.*', 'alerts.*', 'reports.*', 'student_discount.*', 'instagram_feed.*'],
        ],
        'developer' => [
            'name' => '开发人员',
