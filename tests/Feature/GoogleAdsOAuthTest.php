...
class GoogleAdsOAuthTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        config()->set('services.google_ads.redirect_uri', 'http://localhost:8000/store-settings/credentials/google-ads/oauth/callback');
        config()->set('services.google_ads.state_ttl_minutes', 10);
    }

    public function test_admin_can_start_google_ads_oauth_for_the_current_store(): void
    {
        [$user, $organization, $store] = $this->context('organization-admin');
        $this->credential($store, $user, 'client_id', 'google-client-id');
        $this->credential($store, $user, 'client_secret', 'google-client-secret');

        $response = $this->actingAs($user)
            ->withSession($this->contextSession($organization, $store))
            ->get(route('google-ads.oauth.redirect'));
        ...
        $this->assertSame(config('services.google_ads.redirect_uri'), $query['redirect_uri']);
        $response->assertSessionHas(GoogleAdsOAuthService::SESSION_KEY, fn (array $state): bool => hash_equals($state['state_hash'], hash('sha256', $query['state']))
    ...
    public function test_successful_callback_stores_refresh_token_and_marks_provider_connected(): void
    {
        ...
        $this->credential($store, $user, 'client_id', 'google-client-id');
        $this->credential($store, $user, 'client_secret', 'google-client-secret');
        $authorization = app(GoogleAdsOAuthService::class)->begin($store, $user);
        $session = [
            ...$this->contextSession($organization, $store),
            GoogleAdsOAuthService::SESSION_KEY => $authorization['session'],
        ];

        $this->actingAs($user)->withSession($session)
            ->get(route('google-ads.oauth.callback', [
                'state' => $authorization['state'],
                'code' => 'google-authorization-code',
            ]))
            ->assertRedirect(route('store-settings.credentials'))
            ->assertSessionHas('success', 'Google Ads 已连接。')
            ->assertSessionMissing(GoogleAdsOAuthService::SESSION_KEY);
        ...
        $refreshToken = StoreBusinessCredential::query()
            ...
            ->sole();
        $this->assertSame('google-refresh-token', $refreshToken->credential_value);
        $this->assertStringNotContainsString('google-refresh-token', (string) DB::table('store_business_credentials')->where('id', $refreshToken->id)->value('credential_value'));
        $this->assertStringNotContainsString('google-refresh-token', DB::table('audit_logs')->get()->toJson());

        $this->actingAs($user)->withSession($this->contextSession($organization, $store))
            ->get(route('store-settings.credentials'))
            ->assertOk()
            ->assertInertia(fn (Assert $page) => $page
                ->where('credentialProviders.2.key', 'google_ads')
                ->where('credentialProviders.2.oauth_ready', true)
                ->where('credentialProviders.2.oauth_connected', true)
                ->where('credentialProviders.2.oauth_connect_url', route('google-ads.oauth.redirect')));
    ...
    public function test_viewer_cannot_start_google_ads_oauth(): void
    {
    ...
    public function test_callback_rejects_state_from_another_store(): void
    {
        ...
        $secondStore = $this->addStore($user, $organization, 'EU Store', 'google-ads-eu.myshopify.com');
        $this->credential($firstStore, $user, 'client_id', 'google-client-id');
        $this->credential($firstStore, $user, 'client_secret', 'google-client-secret');
        $authorization = app(GoogleAdsOAuthService::class)->begin($firstStore, $user);
        ...
        $this->actingAs($user)->withSession([
            ...
            ->assertSessionHas('error', 'Google Ads 授权状态已失效，请重新连接。');
    ...
    private function context(string $roleSlug): array
    {
        ...
        $organization = Organization::query()->create(['name' => 'Deco', 'code' => 'deco-google-ads']);
        $organization->users()->attach($user, ['status' => 'active', 'joined_at' => now()]);
        $store = $this->addStore($user, $organization, 'US Store', 'google-ads-us.myshopify.com');
