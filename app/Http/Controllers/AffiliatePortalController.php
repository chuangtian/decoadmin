...
class AffiliatePortalController extends Controller
{
    ...
    {
        $v = $r->validate(['program' => ['required', 'string', 'size:26'], 'name' => ['required', 'string', 'max:120'], 'email' => ['required', 'email:rfc', 'max:254'],
        ...
        return back()->with('portal_message', '申请已收到，请等待店铺审核。');
    }

    public function requestLogin(Request $r)
    {
        $v = $r->validate(['email' => ['required', 'email:rfc', 'max:254'], 'program' => ['required', 'string', 'size:26']]);
    ...
    {
        $v = $r->validate(['token' => ['required', 'string', 'size:64']]);
    ...
    public function profile(Request $r)
    {
        ...
        $v = $r->validate(['country' => ['nullable', 'string', 'max:80'], 'payment_method' => ['required', 'in:paypal,bank,other'],
            'payment_reference' => ['required', 'string', 'max:500']]);
        $this->portal->updateProfile($store, $member->id, $v);

        return back()->with('portal_message', '资料已保存。');
    }
