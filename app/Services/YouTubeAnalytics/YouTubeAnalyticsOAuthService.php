...
class YouTubeAnalyticsOAuthService
{
    public const SESSION_KEY = 'youtube_analytics_oauth_state';

    private const AUTHORIZATION_URL = 'https://accounts.google.com/o/oauth2/v2/auth';

    private const TOKEN_URL = 'https://oauth2.googleapis.com/token';

    private const SCOPE = 'https://www.googleapis.com/auth/youtube.readonly https://www.googleapis.com/auth/yt-analytics.readonly';
    ...
    public function begin(Store $store, User $user): array
    {
        ...
        if ($clientId === '' || $clientSecret === '') {
            throw new YouTubeAnalyticsOAuthException('请先保存 YouTube Client ID 和 Client Secret。');
        }
        ...
        return [
            ...
            'session' => [
                ...
                'redirect_uri' => $redirectUri,
                'expires_at' => now()->addMinutes((int) config('services.youtube_analytics.state_ttl_minutes', 10))->getTimestamp(),
            ],
        ];
    }
    ...
    public function complete(Store $store, User $user, array $query, ?array $stateSession): void
    {
        ...
        if (! $this->validState($store, $user, $plainState, $stateSession)) {
            throw new YouTubeAnalyticsOAuthException('YouTube Analytics 授权状态已失效，请重新连接。');
        }

        if (is_string($query['error'] ?? null)) {
            throw new YouTubeAnalyticsOAuthException('YouTube Analytics 授权已取消，请重新连接。');
        }

        $code = is_string($query['code'] ?? null) ? $query['code'] : '';

        if ($code === '') {
            throw new YouTubeAnalyticsOAuthException('YouTube Analytics 未返回有效授权码，请重新连接。');
        }
        ...
        if ($clientId === '' || $clientSecret === '') {
            throw new YouTubeAnalyticsOAuthException('YouTube Analytics OAuth 凭证已变更，请重新连接。');
        }
        ...
        if ($response->failed() || ! is_array($payload) || ! is_string($payload['access_token'] ?? null)) {
            throw new YouTubeAnalyticsOAuthException('YouTube Analytics 访问令牌交换失败，请重新连接。');
        }
        ...
        if ($refreshToken === '') {
            ...
            throw new YouTubeAnalyticsOAuthException('Google 未返回 Refresh Token，请重新授权。');
        }

        $this->credentials->update($store, 'youtube_analytics', 'refresh_token', $refreshToken, $user);
    }
    ...
    {
        return trim((string) $this->credentials->value($store, 'youtube_analytics', $key));
    }

    private function redirectUri(): string
    {
        $redirectUri = (string) (config('services.youtube_analytics.redirect_uri') ?: route('youtube-analytics.oauth.callback'));
        ...
        if (! filter_var($redirectUri, FILTER_VALIDATE_URL)
            ...
            throw new YouTubeAnalyticsOAuthException('YouTube Analytics OAuth 回调地址无效；远程地址必须使用 HTTPS。');
        }
