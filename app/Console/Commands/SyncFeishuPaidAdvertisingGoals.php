...
{
    protected $signature = 'feishu:sync-paid-advertising-goals
        {--store= : 仅同步指定店铺 ID}
        {--google-only : 仅同步 Google Ads 使用的周报和销售目标表}';
    ...
    public function handle(PaidAdvertisingGoalSyncService $sync): int
    {
        ...
        $result = $this->option('google-only')
            ? $sync->syncGoogleAdsSources($storeId ?: null)
            : $sync->syncConfiguredSources($storeId ?: null);
